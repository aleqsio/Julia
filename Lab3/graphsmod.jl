...
function generate_random_graph()
  # Number of graph nodes.
   ...
   # A = Array{Int64,2}(N, N)
    A = zeros(Int64, N, N)                                                       #OPTYMALIZACJA 2
   # for i=1:N, j=1:N
   #   A[i,j] = 0
   # end
    for i in sample(1:N*N, K, replace=false)
      row, col = ind2sub(size(A), i)
      @inbounds A[row,col] = 1                                                  #OPTYMALIZACJA 4
...
function generate_random_nodes()                                        #OPTYMALIZACJA 5
  nodes = Vector()
  for i= 1:N
    if rand(Bool)
      push!(nodes, get_random_person())
    else
      push!(nodes, get_random_address())
    end
end
  nodes
end
...
function convert_to_graph(A, nodes)
  N = length(nodes)
  graph::Array{GraphVertex, 1} = map(n -> GraphVertex(n, GraphVertex[]), nodes)    #OPTYMALIZACJA 3
  for i = 1:N, j = 1:N
      if A[i,j] == 1
...
function test_graph()
  for i=1:10
    ...
    str = graph_to_str(graph)                                                 #OPTYMALIZACJA 4
    # println(str)
